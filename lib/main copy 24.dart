...
Future<void> _refreshAllMaintenanceReminders() async {
  ...
  // 🔴 فاز ۳: قطع کامل دسترسی و غیرفعال‌سازی آلارم‌ها
  if (!isPremium && tier >= 4) {
    ...
    return; // توقف کامل پردازش آلارم‌ها
  }

  // ۱. 🔴 پاکسازی اول: لغو تمام آلارم‌های قبلی (تا کدهای جدید بلافاصله پاک نشوند)
  await NotificationService.cancelMaintenanceNotification('master');
  for (var item in maintenanceItems) {
    await NotificationService.cancelMaintenanceNotification(item.key);
  }
  ...
  final String btnOpen = translate('btn_open', widget.currentLang);
  final String btnDelete = translate('btn_delete', widget.currentLang);
  final String btnSnooze = translate('btn_snooze', widget.currentLang);

  // ۲. بررسی وضعیت قطعات و تنظیم آلارم‌های جدید
  for (var item in maintenanceItems) {
    ...
    if (item.key == 'tires' && item.tireType == '2season') {
      ...
      if (daysUntilDeadline <= 14) {
        ...
        // ✅ تنظیم آلارم آینده برای لاستیک
        await NotificationService.scheduleMaintenanceReminder(
           itemKey: item.key,
           title: widget.currentLang == 'fa' ? 'یادآوری لاستیک' : 'Tire Reminder',
           body: widget.currentLang == 'fa' ? 'موعد بررسی لاستیک نزدیک است' : 'Tire check time is near',
           firstRun: nextDeadline.subtract(const Duration(days: 14)), 
           repeatDaily: true, // ✅ برای تکرار در روزهای بعد
           payload: item.key,
           btnOpenText: btnOpen,
             btnDeleteText: btnDelete,
             btnSnoozeText: btnSnooze,
        );
      }
    } 
    // ب) بررسی سایر قطعات عادی (بر اساس تاریخ یا کیلومتر)
      ...
      bool isKmDue = false;
      DateTime? preciseDueDate; // ✅ متغیر کمکی برای جلوگیری از ارور اسکوپ (Scope)
      ...
      if (item.serviceDate.trim().isNotEmpty) {
        ...
        if (prevDate.year != 2100) {
          preciseDueDate = prevDate.add(Duration(days: intDays));
      ...
      if (isDateDue || isKmDue) {
        ...
        // ✅ اگر هنوز سررسید نشده و تاریخ دقیقی در آینده دارد، آلارم آن را ثبت کن
        if (preciseDueDate != null && preciseDueDate.isAfter(now)) {
           await NotificationService.scheduleMaintenanceReminder(
             itemKey: item.key,
             title: widget.currentLang == 'fa' ? 'یادآوری سرویس' : 'Service Reminder',
             body: widget.currentLang == 'fa' ? 'موعد ${item.key} نزدیک است' : '${item.key} service is near',
             firstRun: preciseDueDate, 
             repeatDaily: true, // ✅ اگر کاربر در تاریخ مقرر سرویس را انجام نداد، روزهای بعد هم آلارم بدهد
             payload: item.key,
             btnOpenText: btnOpen,
           btnDeleteText: btnDelete,
           btnSnoozeText: btnSnooze,
  ...
  // ۳. ثبت آلارم واحد (Master) در صورت نیاز
  if (anyOverdue) {
    ...
    await NotificationService.scheduleMaintenanceReminder(
      ...
      repeatDaily: true, // ✅ تضمین می‌کند که هر روز ساعت ۹ صبح تا زمان رفع مشکل آلارم بدهد
      payload: 'master',
