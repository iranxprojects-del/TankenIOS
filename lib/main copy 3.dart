...
if (!kIsWeb) {
  ...
  Future<void> _showAlarmDialog(MaintenanceItem item) async {
    ...
    await showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text('Reminder for ${item.title}'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(
                ...
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(
                  labelText: 'Reminder before due date (days)',
              ...
              Text(
                item.serviceDate.isEmpty
                    ? 'Set the service date first so the reminder knows when to start.'
                    : 'Reminder will start ${item.reminderDays} days before the saved date and repeat daily at 09:00.',
              ...
              onPressed: () async {
                ...
                if (item.serviceDate.isNotEmpty) {
                  ...
                  await NotificationService.scheduleMaintenanceReminder(
                    title: item.title,
                    body: 'Reminder for ${item.title} on ${item.serviceDate}',
                    dueDate: dueDate,
                    reminderDays: days,
                  );
                }
  ...
  Widget _buildServiceHistoryList() {
    ...
    return ListView(
      ...
      children: [
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  decoration: const InputDecoration(
                    labelText: 'Authorized workshop / representative',
                    border: OutlineInputBorder(),
                ...
                const SizedBox(height: 8),
                Text(
                  'Fill mileage and service dates from the service booklet or by opening the hood and reading the engine / chassis data.',
                  ...
                  DataCell(
                    SizedBox(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(item.serviceDate.isEmpty ? 'Pick date' : item.serviceDate),
                          IconButton(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TextField(
                            ...
                            controller: TextEditingController(text: item.serviceCenter),
                            onChanged: (value) {
                              item.serviceCenter = value;
                              item.isConfigured = _isMaintenanceItemConfigured(item);
                              _saveMaintenanceData();
                            },
                          ),
        ...
        const SizedBox(height: 12),
        Text(
          'Red rows are incomplete and appear first so you can fill them quickly.',
          style: TextStyle(fontSize: 12, color: Colors.grey[700]),
        ),
      ],
    );
