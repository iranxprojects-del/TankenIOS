...
Widget _buildPriceDetail(String label, double price, {bool isBold = false}) {
 ...
 Future<void> _refreshAllMaintenanceReminders() async {
    // ۱. پاک‌سازی تمام آلارم‌های قبلی برای جلوگیری از تداخل
    for (var item in maintenanceItems) {
      await NotificationService.cancelMaintenanceNotification(item.key);
    }
    await NotificationService.cancelNotification(99999); // پاک‌سازی آلارم گروهی (Master)
    ...
    // ۲. بررسی تک‌تک آیتم‌ها و ساخت پیام‌های چندزبانه
    // for (var item in maintenanceItems) {
    ...
    //     tempBody = daysUntil >= 0
    //         ? translate('service_due_days', widget.currentLang, {'days': daysUntil.toString(), 'item': _maintenanceTitle(item)})
    //         : translate('service_overdue', widget.currentLang, {'item': _maintenanceTitle(item)});
    //   } else if (hasTargetKm) {
    //     final int remainingKm = targetKm - currentKmValue;
    //     tempBody = remainingKm >= 0
    //         ? translate('service_due_km', widget.currentLang, {'km': remainingKm.toString(), 'item': _maintenanceTitle(item)})
    //         : translate('service_check_now', widget.currentLang, {'item': _maintenanceTitle(item)});
    //   } else {
    ...
    // جایگزین بخش بررسی حلقه‌ی for در _refreshAllMaintenanceReminders:
...
for (var item in maintenanceItems) {
  ...
  // ۱. تعیین آستانه هشدار اختصاصی برای هر آیتم (متریک‌های شخصی‌سازی شده)
  int alertDaysThreshold = 30;     // پیش‌فرض ۳۰ روز
  int alertKmThreshold = 1000;     // پیش‌فرض ۱۰۰۰ کیلومتر
  ...
  switch (item.key) {
    case 'oil': // روغن موتور
      ...
      break;
    case 'tuv': // معاینه فنی (فقط تاریخ مهم است)
      alertDaysThreshold = 45;     // فرصت بیشتر برای نوبت‌گیری TUV
      alertKmThreshold = 0;        // کیلومتر بی‌تاثیر است
      break;
    case 'brakes': // لنت ترمز
      alertDaysThreshold = 30;
      alertKmThreshold = 1500;     // حساسیت بیشتر روی کیلومتر لنت
      break;
    case 'timing_belt': // تسمه تایم
      alertDaysThreshold = 45;
      alertKmThreshold = 2500;     // تسمه تایم حیاتی است و باید زودتر اعلام شود
      break;
    default: // سایر موارد پیش‌فرض
      alertDaysThreshold = 30;
      alertKmThreshold = 1000;
  }
  ...
  // ۳. بررسی وضعیت بر اساس آستانه‌های اختصاصی همان آیتم
  if (!hasDueDate && !hasTargetKm) {
    tempBody = translate('service_active', widget.currentLang, {'item': _maintenanceTitle(item)});
  } 
  else if ((hasDueDate && daysUntil < 0) || (hasTargetKm && remainingKm < 0)) {
    // اگر موعد هر کدام که تنظیم شده گذشته باشد
    ...
    // قانون: هر کدام زودتر به آستانه اختصاصی رسید
    ...
    } else if (hasTargetKm && remainingKm <= alertKmThreshold && (!hasDueDate || daysUntil > alertDaysThreshold)) {
      tempBody = translate('service_due_km', widget.currentLang, {'km': remainingKm.toString(), 'item': _maintenanceTitle(item)});
    } else {
      // اگر هر دو رد شده‌اند، اولویت با کیلومتر است
      tempBody = translate('service_due_km', widget.currentLang, {'km': remainingKm.toString(), 'item': _maintenanceTitle(item)});
    }
  } 
  else {
  ...
  // ۴. مدیریت شمارش و ذخیره برای نمایش (جلوگیری از اوررایت شدن بی‌مورد)
  activeCount++;
  
  // اگر یک آیتم واقعاً منقضی شده یا نزدیک به موعد است، آن را به عنوان اولویت نمایش انتخاب کن
  if (tempBody.contains('due') || tempBody.contains('overdue') || singleBody.isEmpty) {
    singleItem = item;
    singleBody = tempBody;
  }
}

    if (activeCount == 0) return; // خروج در صورت عدم وجود آلارم فعال

    // ۳. تنظیم برای ساعت ۹ صبح
    DateTime scheduleTime = DateTime(now.year, now.month, now.day, 9, 0);
    if (!scheduleTime.isAfter(now)) {
      scheduleTime = scheduleTime.add(const Duration(days: 1));
    }

    // ۴. ارسال به سرویس (اگر یکی بود پیام اختصاصی، اگر بیشتر بود پیام گروهی)
    await NotificationService.scheduleMaintenanceReminder(
      itemKey: activeCount == 1 ? singleItem!.key : 'master',
      title: translate('service_reminder_title', widget.currentLang),
      body: activeCount == 1 
          ? singleBody 
          : translate('grouped_service_alert_body', widget.currentLang, {'count': activeCount.toString()}),
      firstRun: scheduleTime,
      repeatDaily: true,
      payload: activeCount == 1 ? singleItem!.key : 'master',
      ...
      children: [
        ...
        SizedBox(
          ...
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              // سطر اول: همیشه عنوان نگهداری/تعمیر را نشان می‌دهد
              Text(
              ...
              // سطر دوم: فیلد متنی کیلومتر را فقط در صورتی نشان می‌دهد که آیتم جزو استثناها نباشد
              if (!hideMileage) ...[
                const SizedBox(height: 6), // فاصله بین عنوان و فیلد متنی
                SizedBox(
                  width: 100,
                  child: TextField(
                    keyboardType: TextInputType.number,
  ...
  //                         Row(
  //   mainAxisAlignment: MainAxisAlignment.start, // چیدمان از ابتدا (یا mainAxisSize: MainAxisSize.min)
  //   mainAxisSize: MainAxisSize.min,
  //   children: [
  //     Row(
  ...
  //       mainAxisSize: MainAxisSize.min,
  //       children: [
  //         // Show website icon only when item has a website AND workshop phone is not being used as primary contact
  ...
  //         // Show phone icon except for TÜV (key 'tuv') and only when there is a phone to call
  //         if (item.key != 'tuv') //&& _getPrimaryPhoneNumber(item).isNotEmpty|| _isValidPhoneNumber(workshopPhone))
  //           IconButton(
  //             padding: EdgeInsets.zero,
  //             constraints: const BoxConstraints(),
                        ...
                        children: [
                          Row(
                            children: [
                              GestureDetector(
  onTap: () async {
    // اگر کاربر در حال روشن کردن آلارم است
    if (!item.alarmEnabled) {
      if (item.key == 'oil') {
        // برای روغن: در صورت خالی بودن، مقادیر پیش‌فرض ست می‌شوند
        bool updated = false;
        if (item.mileage.trim().isEmpty) {
          int curKm = int.tryParse(currentKm) ?? 0;
          item.mileage = (curKm + 15000).toString();
          updated = true;
        }
        if (item.serviceDate.trim().isEmpty) {
          DateTime nextYear = DateTime.now().add(const Duration(days: 365));
          item.serviceDate = _formatMaintenanceDate(nextYear);
          updated = true;
        }
        if (updated) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                widget.currentLang == 'fa'
                    ? 'مقادیر پیش‌فرض (۱۵۰۰۰ کیلومتر و ۱ سال) برای روغن ثبت شد.'
                    : 'Default values (15000 km, 1 year) applied for oil.',
              ),
              backgroundColor: Colors.blue.shade700,
            ),
          );
        ...
        // برای بقیه موارد: حتماً باید حداقل یکی از مقادیر کیلومتر یا تاریخ پر شده باشد
        if (item.mileage.trim().isEmpty && item.serviceDate.trim().isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(translate('set_date_first', widget.currentLang)),
              backgroundColor: Colors.red.shade700,
            ),
          );
          return; // خروج و جلوگیری از فعال شدن آلارم
        }
      }
    }
